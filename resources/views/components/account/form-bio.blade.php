...
<div x-data="{ 
    value: {{ json_encode($user->bio ?? '') }},
    loading: false,
    errors: {}
}">
    <form @submit.prevent="loading = true; errors = {};
            fetch('{{ route('account.update-bio') }}', {
                method: 'PATCH',
                headers: {
        ...
        <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-start gap-2 border-t border-gray-100 dark:border-gray-800">
            ...
            <div class="flex-1">
                <div class="flex gap-2">
                    <textarea x-model="value" 
                              rows="3"
                              :class="errors.bio ? 'border-red-300 dark:border-red-700' : 'border-gray-200 dark:border-gray-700'"
                    ...
                    <button type="submit" 
                            class="px-4 py-2 bg-bluefilterpedia text-white text-sm font-medium rounded-lg hover:opacity-90 transition-opacity whitespace-nowrap disabled:opacity-50 min-w-[80px] self-start"
                            :disabled="loading">
                ...
                <template x-if="errors.bio">
                    <p class="text-xs text-red-500 mt-1" x-text="errors.bio[0]"></p>
                </template>
            </div>
        </div>
    </form>
