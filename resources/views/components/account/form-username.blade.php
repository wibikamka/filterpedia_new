...
<div x-data="{ 
    ...
    <form @submit.prevent="loading = true; errors = {};
            fetch('{{ route('account.update-username') }}', {
                method: 'PATCH',
                headers: {
        ...
        <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-start gap-2 border-t border-gray-100 dark:border-gray-800">
            ...
            <div class="flex-1">
                <div class="flex gap-2">
                    <input type="text" 
                           x-model="value"
                           :class="errors.username ? 'border-red-300 dark:border-red-700' : 'border-gray-200 dark:border-gray-700'"
                ...
                <template x-if="errors.username">
                    <p class="text-xs text-red-500 mt-1" x-text="errors.username[0]"></p>
                </template>
            </div>
        </div>
    </form>
